...
class SettingsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UIPickerViewDelegate, UIPickerViewDataSource  {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row < 3 {
            cell = tableView.dequeueReusableCell(withIdentifier: "idCellSlider", for: indexPath as IndexPath) as UITableViewCell
    ...
    func prepareVoiceList() {
        for voice in AVSpeechSynthesisVoice.speechVoices() {
            ...
            let languageName = NSLocale.current.localizedString(forIdentifier: voiceLanguageCode)
          //  displayNameForKey(NSLocaleIdentifier, value: voiceLanguageCode)
